...
export default function ContactForm() {
  ...
  const inputClass = 'w-full rounded-xl border border-[#e3e2dd] bg-white px-4 py-3 text-sm text-[#1e1716] placeholder-[#999] outline-none transition focus:border-[#A392DD] focus:ring-1 focus:ring-[#A392DD]/30 font-poppins'

  if (status === 'sent') return (
    <div className='rounded-2xl border border-[#A392DD]/20 bg-[#A392DD]/10 p-8 text-center'>
      <p className='mb-2 font-playfair text-2xl text-[#1e1716]'>Message received ✓</p>
  ...
  return (
    <form onSubmit={handleSubmit} className='space-y-4'>
      ...
      <div>
        ...
        <textarea name='message' required rows={5} value={form.message} onChange={handleChange} className={inputClass + ' resize-none'} placeholder='What is on your mind?' />
      </div>
      ...
      {status === 'error' && <p className='text-center text-sm text-red-500'>Something went wrong. Try emailing us directly.</p>}
    </form>
  )
}
