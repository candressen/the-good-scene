...
export default function Nav() {
  ...
  return (
    <header className='fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-2rem)] max-w-[1200px] px-0 font-poppins text-[#1e1716]'>
      ...
      {open && (
        <div id='mobile-nav' className='mt-2 rounded-2xl border border-[#e3e2dd] bg-white px-6 py-4 md:hidden'>
          <div className='flex flex-col gap-4'>
            ...
            <div className='flex flex-col gap-3 pt-2'>
              <Link href='/events' onClick={() => setOpen(false)}
                className='rounded-full bg-[#1e1716] px-6 py-2.5 text-center text-sm font-medium text-white'>
                Get Started
              </Link>
              <Link href='/contact' onClick={() => setOpen(false)}
                className='rounded-full border-2 border-[#1e1716] px-6 py-2.5 text-center text-sm font-medium text-[#1e1716]'>
                Contact Us
              </Link>
            </div>
          </div>
